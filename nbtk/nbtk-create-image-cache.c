...
static int get_effective_Y(int X1, int X2)
{
	int Y = 0;
	GList *item;
	struct imgcache_element *element;
...
static inline uint32_t get_pixel(struct imgcache_element *element, int X, int Y)
{
	GdkPixbuf *buf;
	unsigned char *p;
	buf = element->ptr;
...
static int can_scooch(struct imgcache_element *one, struct imgcache_element *two)
{
	int i,x,y;
...
static void makecache(char *directory, int recurse)
{
	DIR *dir;
...
static void write_cache_file(char *directory, char *pngfile)
{
	...
	sprintf(filename, "%s/nbtk.cache", directory);
...
int main(int argc, char **argv)
{
	...
	makecache(argv[1], 1);
	optimal_placement();
	sprintf(image_file, "/var/cache/nbtk/%08x.png", g_str_hash(argv[1]));
